      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-[#C5A059]/20 pb-4">
        <div>
          ...
          <h3 className="font-serif text-lg sm:text-xl text-[#141312]">
            {subMode === "camera" ? "Live AR Mirror" : "Upload a photo or capture a live selfie"}
          </h3>
        </div>
              ...
              <div className="relative w-full max-w-sm aspect-[3/4] rounded-2xl overflow-hidden border-2 border-[#C5A059]/60 shadow-xl bg-black flex items-center justify-center">
                ...
                <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-center overflow-hidden">
                  <motion.div
                    ...
                    className="pointer-events-auto cursor-move relative z-10 p-4 drop-shadow-2xl"
                  >
                    <Image
                      src={`/api/isolate-jewelry?url=${encodeURIComponent(productImageUrl)}`}
                      alt="Live Try-On"
                      width={180}
                      height={180}
                      className="object-contain filter drop-shadow-lg"
                      unoptimized // the API returns a precise transparent PNG, let's not let NextJS convert it to WebP and mess up alpha sometimes
                    />
                  </motion.div>
                </div>
                ...
                <button
                  ...
                  className="absolute top-3 right-3 p-2.5 rounded-full bg-black/60 backdrop-blur-xs text-white hover:bg-black transition-colors z-20"
                  title="Switch camera"
                >
                  <RefreshCw size={15} />
                </button>
                ...
                <div className="absolute bottom-4 left-0 right-0 text-center pointer-events-none z-20">
                  <span className="bg-black/60 backdrop-blur-md text-white text-[10px] font-sans px-3 py-1.5 rounded-full uppercase tracking-widest shadow-lg border border-white/10">
                    Drag jewelry to align
                  </span>
                </div>
              </div>

              {/* No shutter button needed for Live AR */}
            </div>
          )}
