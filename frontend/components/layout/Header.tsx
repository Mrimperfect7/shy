...
export default function Header({ isLoggedIn: serverLoggedIn = false, showroom = false }: { isLoggedIn?: boolean; showroom?: boolean }) {
  ...
  return (
    <>
      {!isHomepage && (showroom ? <div className="h-16 lg:h-18" aria-hidden /> : <div style={{ height: "calc(2.5rem + 4.2rem)" }} aria-hidden />)}
      {isHomepage && scrolled && <div style={{ height: "calc(2.5rem + 4.2rem)" }} aria-hidden />}
      ...
      <header
        ...
        className={`site-header transition-all duration-300 ${
          isTransparent
            ...
            : scrolled
            ? "scrolled shadow-sm bg-[#FAF8F5]/95 backdrop-blur-md"
            : "bg-[#FAF8F5]/90 backdrop-blur-md"
        }`}
        style={{ top: showroom ? 0 : "2.5rem" }}
      >
        <div className="max-w-7xl mx-auto px-5 lg:px-10">
          <div className="flex items-center justify-between h-16 lg:h-18">
            ...
            >
              <Logo
                variant="horizontal"
                theme={isTransparent ? "white" : "dark"}
                priority
            ...
            <nav
              ...
              className={`hidden lg:flex items-center gap-4 xl:gap-6 flex-shrink-0 transition-opacity duration-300 ${
            ...
            {/* Right: Actions & Cart */}
            <div className={`flex items-center gap-1.5 sm:gap-3 transition-opacity duration-300 ${isTransparent ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
              <button
                id="search-btn"
