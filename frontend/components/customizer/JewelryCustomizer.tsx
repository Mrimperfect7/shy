...
import { ArrowLeft, Hand, Gem, CircleDot, RotateCcw } from "lucide-react";
...
const TryOnScene = dynamic(() => import("./TryOnScene"), {
  ...
  loading: () => (
    <div className="absolute inset-0 flex items-center justify-center bg-[#0B0B0C]">
      <p data-testid="showroom-loading" className="font-serif text-lg text-amber-200/70 animate-pulse tracking-wide">Preparing the showroom…</p>
    </div>
  ),
});
...
.tryon-vignette{background:radial-gradient(ellipse at 50% 42%,transparent 45%,rgba(5,5,6,0.4) 100%)}`}</style>
      <div
        data-testid="customizer-root"
        className="h-[calc(100dvh-4rem)] lg:h-[calc(100dvh-4.5rem)] bg-[#0B0B0C] text-[#F7F5F0] flex flex-col overflow-hidden"
      >
        {/* showroom header */}
        ...
        <div data-lenis-prevent className="flex-1 flex flex-col lg:flex-row min-h-0 overflow-y-auto lg:overflow-hidden">
          {/* 3D viewport */}
          <div className="relative flex-[1.25] lg:flex-1 min-h-0 order-1 bg-[#151619]" data-testid="viewport-3d">
            <TryOnScene />
            {/* cinematic vignette */}
            ...
            <button
              data-testid="reset-model-view"
              aria-label="Reset model view"
              title="Reset model view"
              onClick={() => setCameraView(cameraView)}
              className="absolute top-4 right-4 z-20 flex items-center gap-2 rounded-full border border-white/15 bg-black/40 px-3 py-2 text-[10px] uppercase tracking-widest text-zinc-300 backdrop-blur-sm transition-colors hover:bg-white/10 hover:text-white"
            >
          ...
          {/* catalog sidebar */}
          <div className="order-2 w-full lg:w-[380px] xl:w-[420px] flex-1 lg:flex-none min-h-0 lg:h-auto border-t lg:border-t-0 border-[#D4AF37]/10">
